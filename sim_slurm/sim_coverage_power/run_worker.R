...
##
## NOTE: This file was generated by Claude (AI-assisted).
##
...
## --- Record the hardware this task ran on ------------------------------------
## Per-core throughput varies a lot across the cluster's node generations, and
## the rt1/rt2 columns are only interpretable alongside the CPU they were
## measured on. Capture host + CPU model once and stamp them on every row.
host_name <- Sys.info()[["nodename"]]
cpu_model <- tryCatch(
    trimws(sub("^model name\\s*:\\s*", "",
...
run_sim <- function(s) {
    ...
    data.frame(
        ...
        n_cov      = SIM_CFG$n_cov,
        n_rand     = SIM_CFG$n_rand,
        # -- Hardware this row was computed on (for interpreting rt1/rt2) -------
        host       = host_name,
        cpu_model  = cpu_model,
