...
#
# run_AR() is the user-facing wrapper: it standardizes columns, (optionally)
...
test_that("run_AR maps custom y/d/z names", {
  df <- make_df()
  names(df) <- c("out", "took", "z", "x1", "x2")
  quiet(res <- run_AR(df, y = "out", d = "took", z = "z", seed = 1, n_rand = NR))
...
test_that("x = NULL (default) uses no covariates, even with extra columns", {
  ...
  quiet(res <- run_AR(make_df(), seed = 1, n_rand = NR))
...
test_that("x names covariates -> both without- and with-covariates versions", {
  quiet(res <- run_AR(make_df(), x = c("x1", "x2"), seed = 1, n_rand = NR))
...
test_that("x can select a subset of columns as covariates", {
  quiet(res <- run_AR(make_df(), x = "x1", seed = 1, n_rand = NR))
...
test_that("with_covariate = FALSE prepares only the without-cov version", {
  quiet(res <- run_AR(make_df(), x = c("x1", "x2"),
...
test_that("x = character(0) is treated as no covariates", {
  quiet(res <- run_AR(make_df(), x = character(0), seed = 1, n_rand = NR))
  expect_length(res$covariates, 0)
  expect_equal(res$runs, "without")
})

test_that("non-character x is rejected", {
  expect_error(run_AR(make_df(), x = 1:2), "character vector")
})

test_that("duplicate names in x are rejected", {
  expect_error(run_AR(make_df(), x = c("x1", "x1")), "duplicate")
})

test_that("x naming a missing column errors", {
  expect_error(run_AR(make_df(), x = c("x1", "nope")), "not found")
})

test_that("x overlapping the y/d/z columns errors", {
  expect_error(run_AR(make_df(), x = c("x1", "assignment")), "must not name")
})
...
test_that("a demeaning message is printed and names the covariates", {
  out <- utils::capture.output(
    res <- run_AR(make_df(), x = c("x1", "x2"), seed = 1, n_rand = NR))
...
test_that("non-numeric covariates are rejected", {
  ...
  expect_error(run_AR(df, x = "grp"), "numeric")
})

# --- Setup messages ----------------------------------------------------------

test_that("setup message reports N, N1, N0, draws and compliance", {
  out <- utils::capture.output(res <- run_AR(make_df(n = 40), n_rand = 50,
                                             seed = 1))
...
test_that("guard stops on a perfectly collinear covariate (rank deficient)", {
  ...
  expect_error(run_AR(df, x = c("x1", "x2")),
...
test_that("guard stops on a near-collinear covariate via condition number", {
  ...
  expect_error(run_AR(df, x = c("x1", "x2"), cond_threshold = 1e6),
...
test_that("guard is skipped (NULL) when no with-covariates run happens", {
  quiet(res <- run_AR(make_df(), seed = 1, n_rand = NR))  # x = NULL
  expect_null(res$guard)
})
...
# --- Wiring: run_AR runs the algorithm and returns CS + p_value --------------

test_that("run_AR returns CS + p_value per version", {
  quiet(res <- run_AR(make_late_df(), algorithm = "algo2", n_rand = 60,
                      seed = 1))
...
test_that("run_AR result matches calling the algorithm directly", {
  # run_AR returns the prepared inputs and zsim it used, so a direct call on
...
test_that("run_AR dispatches to algo1 when requested", {
  quiet(res <- run_AR(make_late_df(), algorithm = "algo1", n_rand = 60,
                      seed = 1))
  expect_equal(res$algorithm, "algo1")
  expect_named(res$results$without, c("confidence_set", "p_value"))
})

test_that("run_AR with covariates returns results for both versions", {
  df <- make_late_df()
  df$w <- rnorm(nrow(df))   # a covariate
  quiet(res <- run_AR(df, x = "w", algorithm = "algo2", n_rand = 60, seed = 1))
  expect_equal(res$runs, c("without", "with"))
  expect_named(res$results$without, c("confidence_set", "p_value"))
